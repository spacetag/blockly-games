...
Blockly.Blocks['procedures_callnoreturn'] = {
  ...
  onchange: function(event) {
    ...
    if (event.type == Blockly.Events.BLOCK_CREATE &&
      ...
      if (!def) {
         ...
         *     <value name="return_value_what_is_this_really_called_?">
         *       <block type="blank_expression"></block>
         *     <value>
         *   </block>
         * </xml>
         */
        ...
        var blankBlock = goog.dom.createDom('block');
        blankBlock.setAttribute('type', 'blank_expression');
        block.setAttribute('type', this.defType_);
        value.setAttribute('name', 'RETURN')
        value.appendChild(blankBlock)
        var xy = this.getRelativeToSurfaceXY();
        var x = xy.x + Blockly.SNAP_RADIUS * (this.RTL ? -1 : 1) + 150;
        var y = xy.y + Blockly.SNAP_RADIUS * 2 - 50;
        ...
        var blockIds = Blockly.Xml.domToWorkspace(xml, this.workspace);
        var realBlock = this.workspace.getBlockById(blockIds[0])
        realBlock.svgGroup_.style.visibility = "hidden"
  ...
  customContextMenu: function(options) {
    var option = {enabled: true};
    option.text = (this.defWasVisible ? "Hide" : "Show") + " pseudocode definition";
    var name = this.getProcedureCall();
    var workspace = this.workspace;
    var callBlock = this;
    option.callback = function() {
      console.log(callBlock.defWasVisible);
      if (callBlock.defWasVisible) {
        callBlock.defWasVisible = false;
      } else {
        callBlock.defWasVisible = true;
      }
      ...
      if (def) {
        ...
        def.moveBy(x, y)
        def.svgGroup_.style.visibility = callBlock.defWasVisible ? "" : "hidden";
